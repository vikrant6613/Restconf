...
response_put = requests.put(url, headers=device['headers'], auth=(device['username'], device['password']), verify=False, data=payload)

response_get = requests.get(url, headers=device['headers'], auth=(device['username'], device['password']), verify=False).json()
